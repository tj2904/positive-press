...
VS Code test execution is configured to use `scripts/run-jest.sh` via workspace settings. The launcher script:

- Loads `nvm` when available (`$HOME/.nvm/nvm.sh`)
- Resolves `node` from the current machine environment
- Runs the workspace Jest binary (`node_modules/jest/bin/jest.js`)

This avoids hard-coding a machine-specific Node path in `.vscode/settings.json` while keeping the VS Code Test UI working across different developer setups.
